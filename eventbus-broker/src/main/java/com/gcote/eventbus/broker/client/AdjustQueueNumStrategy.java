...
package com.gcote.eventbus.broker.client;
...
public class AdjustQueueNumStrategy {
    ...
    public AdjustQueueNumStrategy(final EventBrokerController eventBrokerController) {
    ...
    private void adjustQueueNumByConsumerCount(String topic, AdjustType scaleType) {
        if (BrokerRole.SLAVE == this.eventBrokerController.getMessageStoreConfig().getBrokerRole()) {
        ...
        switch (scaleType) {
            ...
            case DECREASE_QUEUE_NUM:
                ...
                long delayTimeMinutes = Math.min(eventBrokerController.getEventBusBrokerConfig().getScaleQueueSizeDelayTimeMinute(), 10);
    ...
    private void adjustReadQueueNumByConsumerCount(String topic, long delayMills, AdjustType mode) {
        Runnable scaleQueueTask = new Runnable() {
            ...
            @Override
            public void run() {
                TopicConfig topicConfig = eventBrokerController.getTopicConfigManager().getTopicConfigTable().get(topic);
                if (topicConfig != null) {
                    synchronized (topicConfig) {

                        //query again to ensure it's newest
                        topicConfig = eventBrokerController.getTopicConfigManager().getTopicConfigTable().get(topic);
                        ...
                        if (adjustReadQueueSize != topicConfig.getReadQueueNums()) {
                            ...
                            if (canAdjustReadQueueSize) {
                                if (adjustReadQueueSize >= topicConfig.getWriteQueueNums() && adjustReadQueueSize < 1024) {
                                    ...
                                    eventBrokerController.getTopicConfigManager().updateTopicConfig(topicConfigNew);
                                    eventBrokerController.registerBrokerAll(true, false, true);
                                ...
                                if (this.alreadyRetryTimes < eventBrokerController.getEventBusBrokerConfig().getScaleQueueRetryTimesMax()) {
    ...
    private void scheduleAdjustQueueSizeTask(Runnable task, long delay, String topic, AdjustType mode) {
        ...
        if (queueSize < this.eventBrokerController.getEventBusBrokerConfig().getScaleQueueThreadPoolQueueCapacity()) {
    ...
    private int adjustQueueSizeByMaxConsumerCount(String topic) {
        int queueSize = this.eventBrokerController.getEventBusBrokerConfig().getMinQueueNum();
        Set<String> maxCidList = null;
        Set<String> topicConsumeByWho = this.eventBrokerController.getConsumerManager().queryTopicConsumeByWho(topic);
        for (String group : topicConsumeByWho) {
            EventConsumerGroupInfo consumerGroupInfo = (EventConsumerGroupInfo) this.eventBrokerController.getConsumerManager().getConsumerGroupInfo(group);
    ...
    private long nearbyClients(Set<String> cidList) {
        long locClient = cidList.stream().filter(new Predicate<String>() {
            ...
            public boolean test(String cid) {
                String[] cidArr = cid.split(EventBusConstant.INSTANCE_NAME_SEPERATER);
                if (cidArr.length > 2) {
                    String idc = cidArr[cidArr.length - 1];
                    String clusterName = eventBrokerController.getBrokerConfig().getBrokerClusterName();
    ...
    public void notifyWhenTopicConfigChange(String topic) {
        Set<String> topicConsumeByWho = this.eventBrokerController.getConsumerManager().queryTopicConsumeByWho(topic);
        for (String group : topicConsumeByWho) {
            ConsumerGroupInfo consumerGroupInfo = this.eventBrokerController.getConsumerManager().getConsumerGroupInfo(group);
            if (consumerGroupInfo != null) {
                List<Channel> channelList = consumerGroupInfo.getAllChannel();
                for (Channel channel : channelList) {
                    this.eventBrokerController.getEventBusBroker2Client().notifyWhenTopicConfigChange(channel, topic);
                }
            }
        }
    }
    ...
    public boolean isCanAdjustReadQueueSize(String topic, int scaleQueueSize) {
        ...
        if (topicConfig != null) {
            for (int qId = scaleQueueSize; qId < topicConfig.getReadQueueNums(); qId++) {
                long maxOffsetInConsumeQueue = eventBrokerController.getMessageStore().getMaxOffsetInQueue(topic, qId);
                long lastMsgTime = eventBrokerController.getMessageStore().getMessageStoreTimeStamp(topic, qId, maxOffsetInConsumeQueue - 1);
                ...
                Set<String> topicConsumeByWho = this.eventBrokerController.getConsumerManager().queryTopicConsumeByWho(topic);
                Set<String> groupInOffset = this.eventBrokerController.getConsumerOffsetManager().whichGroupByTopic(topic);
    ...
    private boolean isAllMessageConsumed(String topic, Set<String> groups, int queueId) {
        for (String group : groups) {
            long maxOffset = eventBrokerController.getMessageStore().getMaxOffsetInQueue(topic, queueId);
            long ackOffset = eventBrokerController.getConsumeQueueManager().queryOffset(group, topic, queueId);
