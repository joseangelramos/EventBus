...
package com.gcote.eventbus.broker.consumequeue;
...
public class ConsumeQueueManager {
    ...
    private static ConsumeQueueManager eventQueueManager = new ConsumeQueueManager();
    ...
    public EventBrokerController getBrokerController() {
        return eventBrokerController;
    }
    ...
    public long getMaxQueueDepth(String topic) {
        return this.eventBrokerController.getExtTopicConfigManager().selectExtTopicConfig(topic).getMaxQueueDepth();
    }

    public void setBrokerController(EventBrokerController brokerController) {
        this.eventBrokerController = brokerController;
    }

    //update accumulated depth
    public void load() {
        eventBrokerController.scheduleTaskAtFixedRate(() -> {
        ...
        }, 1000, eventBrokerController.getEventBusBrokerConfig().getDepthCheckInterval());
    }
    ...
    public ConsumeQueueWaterMark calculateMinAccumulated(String topic, int queueId) {
        Set<String> subscribedGroups = eventBrokerController.getConsumerOffsetManager().whichGroupByTopic(topic);
        Set<String> checkGroups = new HashSet<String>();
        EventBusTopicConfig eventBusTopicConfig = this.getBrokerController().getExtTopicConfigManager().selectExtTopicConfig(topic);
        long maxDepth = eventBusTopicConfig != null ? eventBusTopicConfig.getMaxQueueDepth() : EventBusTopicConfig.DEFAULT_QUEUE_LENGTH;
        ...
        long maxOffset = this.eventBrokerController.getMessageStore().getMaxOffsetInQueue(topic, queueId);
        ...
        for (String consumerGroup : subscribedGroups) {
            if (topic.startsWith(MixAll.DLQ_GROUP_TOPIC_PREFIX) || topic.startsWith(MixAll.RETRY_GROUP_TOPIC_PREFIX) || consumerGroup.startsWith(EventBusConstant.EXT_CONSUMER_GROUP)) {
                continue;
            }

            EventConsumerManager consumerManager = (EventConsumerManager) this.eventBrokerController.getConsumerManager();
            EventConsumerGroupInfo eventConsumerGroupInfo = (EventConsumerGroupInfo) consumerManager.getConsumerGroupInfo(consumerGroup);

            //ignore offline consumer group
            if (eventConsumerGroupInfo == null || eventConsumerGroupInfo.getClientIdBySubscription(topic) == null
                || eventConsumerGroupInfo.getClientIdBySubscription(topic).isEmpty()) {
                continue;
            }
    ...
    private void autoUpdateDepth(String consumerGroup, String topic, int queueId, long maxDepth, long maxOffset) {
        if (!this.eventBrokerController.getEventBusBrokerConfig().isAutoUpdateDepth()) {
            return;
        }

        EventConsumerManager consumerManager = this.eventBrokerController.getConsumerManager();
        EventConsumerGroupInfo eventConsumerGroupInfo = (EventConsumerGroupInfo) consumerManager.getConsumerGroupInfo(consumerGroup);
        ...
        this.eventBrokerController.getConsumeQueueManager().recordLastDeliverOffset(consumerGroup, topic, queueId, nowDeliverOffset);
        ...
        if (this.eventBrokerController.getConsumerOffsetManager().queryOffset(consumerGroup, topic, queueId) != -1) {
            this.eventBrokerController.getConsumerOffsetManager().commitOffset("resetByBroker", consumerGroup, topic, queueId, nowDeliverOffset);
        } else {
    ...
    public long queryOffset(final String group, final String topic, final int queueId) {
        String key = topic + TOPIC_GROUP_SEPARATOR + group;
        ConcurrentMap<Integer, Long> map = this.eventBrokerController.getConsumerOffsetManager().getOffsetTable().get(key);
        if (null != map) {
    ...
    public void resetOffsetOnClient(String consumerGroup, String topic, int queueId, long maxDepth, long maxOffset) {
        ...
        TopicConfig topicConfig = eventBrokerController.getTopicConfigManager().selectTopicConfig(topic);

        if (queueId < topicConfig.getWriteQueueNums()) {
            MessageQueue mq = new MessageQueue(topic, eventBrokerController.getBrokerConfig().getBrokerName(), queueId);
        ...
        ConsumerGroupInfo consumerGroupInfo =
            this.eventBrokerController.getConsumerManager().getConsumerGroupInfo(consumerGroup);
        ...
        if (consumerGroupInfo != null && !consumerGroupInfo.getAllChannel().isEmpty()) {
            ...
            for (Map.Entry<Channel, ClientChannelInfo> entry : channelInfoTable.entrySet()) {
                ...
                if (version >= MQVersion.Version.V3_0_7_SNAPSHOT.ordinal()) {
                    try {
                        this.eventBrokerController.getRemotingServer().invokeOneway(entry.getKey(), request, 5000);
