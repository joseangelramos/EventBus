...
import com.gcote.eventbus.broker.plugin.EventPluginMessageStore;
import com.gcote.eventbus.common.EventBusConstant;
import com.gcote.eventbus.common.message.EventBusMessageConst;
...
public class EventReplyMessageProcessor extends AbstractSendMessageProcessor implements NettyRequestProcessor {
    private EventBrokerController eventBrokerController;

    public EventReplyMessageProcessor(final EventBrokerController eventBrokerController) {
        super(eventBrokerController);
    ...
    public RemotingCommand processRequest(ChannelHandlerContext ctx,
        ...
        switch (request.getCode()) {
            case EventBusRequestCode.SEND_DIRECT_MESSAGE_V2:
            case EventBusRequestCode.SEND_DIRECT_MESSAGE:
    ...
    public boolean rejectRequest() {
        return this.eventBrokerController.getMessageStore().isOSPageCacheBusy();
    }
    ...
    private RemotingCommand processReplyMessageRequest(final ChannelHandlerContext ctx, //
        ...
        response.addExtField(MessageConst.PROPERTY_MSG_REGION, this.eventBrokerController.getBrokerConfig().getRegionId());

        log.debug("receive SendDirectMessage request command, " + request);

        final long startTimestamp = this.eventBrokerController.getBrokerConfig().getStartAcceptSendRequestTimeStamp();
        if (this.eventBrokerController.getMessageStore().now() < startTimestamp) {
        ...
        TopicConfig topicConfig = this.eventBrokerController.getTopicConfigManager().selectTopicConfig(requestHeader.getTopic());
        ...
        if ((null != newTopic && newTopic.startsWith(MixAll.RETRY_GROUP_TOPIC_PREFIX))) {
            ...
            SubscriptionGroupConfig subscriptionGroupConfig =
                this.eventBrokerController.getSubscriptionGroupManager().findSubscriptionGroupConfig(groupName);
            ...
            if (reconsumeTimes >= maxReconsumeTimes) {
                ...
                topicConfig = this.eventBrokerController.getTopicConfigManager().createTopicInSendMessageBackMethod(newTopic, //
                    DLQ_NUMS_PER_GROUP, //
        ...
        if (this.eventBrokerController.getBrokerConfig().isRejectTransactionMessage()) {
            String traFlag = msgInner.getProperty(MessageConst.PROPERTY_TRANSACTION_PREPARED);
            if (traFlag != null) {
                response.setCode(ResponseCode.NO_PERMISSION);
                response.setRemark(
                    "the broker[" + this.eventBrokerController.getBrokerConfig().getBrokerIP1() + "] sending transaction constant is forbidden");
                return response;
            }
        }
        ...
        if (msgInner.getProperties() != null && EventBusConstant.REPLY.equals(msgInner.getProperties().get(EventBusConstant.KEY))) {
            String senderId = msgInner.getProperties().get(EventBusConstant.PROPERTY_MESSAGE_REPLY_TO);
            if (senderId == null) {
                log.warn("senderId is null, can not reply message");
            } else {
                ClientChannelInfo clientChannelInfo = this.eventBrokerController.getProducerManager().getClientChannel(senderId);
                if (clientChannelInfo == null || clientChannelInfo.getChannel() == null || !clientChannelInfo.getChannel().isActive()) {
                    ...
                    map.put(EventBusMessageConst.LEAVE_TIME, String.valueOf(System.currentTimeMillis()));
                    replyMessageRequestHeader.setProperties(MessageDecoder.messageProperties2String(map));

                    try {
                        this.eventBrokerController.getPushReplyMessageExecutor().submit(new Runnable() {
                            @Override public void run() {
                                boolean isPushSuccess = eventBrokerController.getEventBusBroker2Client().pushRRReplyMessageToClient
                                    (clientChannelInfo.getChannel(), replyMessageRequestHeader, msgInner);
                                if (isPushSuccess) {
                                    eventBrokerController.getBrokerStatsManager().incBrokerGetNums(1);
                                    if (eventBrokerController.getMessageStore() instanceof EventPluginMessageStore) {
                                        DefaultMessageStore defaultMessageStore = (DefaultMessageStore)
                                            ((EventPluginMessageStore) eventBrokerController.getMessageStore()).getDefaultMessageStore();
        ...
        PutMessageResult putMessageResult = this.eventBrokerController.getMessageStore().putMessage(msgInner);
        ...
        if (putMessageResult != null) {
            ...
            if (sendOK) {
                ...
                this.eventBrokerController.getBrokerStatsManager().incTopicPutNums(msgInner.getTopic());
                this.eventBrokerController.getBrokerStatsManager().incTopicPutSize(msgInner.getTopic(),
                    putMessageResult.getAppendMessageResult().getWroteBytes());
                this.eventBrokerController.getBrokerStatsManager().incBrokerPutNums();
                ...
                if (hasSendMessageHook()) {
                    ...
                    int commercialBaseCount = eventBrokerController.getBrokerConfig().getCommercialBaseCount();
    ...
    private String diskUtil() {
        String storePathPhysic = this.eventBrokerController.getMessageStoreConfig().getStorePathCommitLog();
        double physicRatio = UtilAll.getDiskPartitionSpaceUsedPercent(storePathPhysic);

        String storePathLogis =
            StorePathConfigHelper.getStorePathConsumeQueue(this.eventBrokerController.getMessageStoreConfig().getStorePathRootDir());
        double logisRatio = UtilAll.getDiskPartitionSpaceUsedPercent(storePathLogis);

        String storePathIndex =
            StorePathConfigHelper.getStorePathIndex(this.eventBrokerController.getMessageStoreConfig().getStorePathRootDir());
    ...
    public void processRequestLater(ChannelHandlerContext ctx, RemotingCommand request) {
        ScheduledThreadPoolExecutor threadPoolExecutor = this.eventBrokerController.getSendReplyScheduledExecutorService();
        if (threadPoolExecutor.getQueue().size() > this.eventBrokerController.getEventBusBrokerConfig().getSendReplyThreadPoolQueueCapacity()) {
        ...
        this.eventBrokerController.getSendReplyScheduledExecutorService().schedule(new Runnable() {
            @Override
            public void run() {
                eventBrokerController.getSendReplyMessageExecutor().submit(new Runnable() {
                    @Override
                    public void run() {
                        try {
                            EventReplyMessageProcessor.this.processRequest(ctx, request);
                        } catch (Throwable e) {
