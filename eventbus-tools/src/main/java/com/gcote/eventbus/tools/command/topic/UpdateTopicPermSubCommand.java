...
package com.gcote.eventbus.tools.command.topic;

import com.gcote.eventbus.tools.admin.EventBusAdminExt;
...
public class UpdateTopicPermSubCommand implements SubCommand {
    ...
    public void execute(final CommandLine commandLine, final Options options, RPCHook rpcHook) {
        EventBusAdminExt eventBusAdminExt = new EventBusAdminExt(rpcHook, 3000L);
        eventBusAdminExt.setInstanceName(Long.toString(System.currentTimeMillis()));
        try {
            eventBusAdminExt.start();
            ...
            for (String topic : topicList) {
                try {
                    updateTopicPerm(topic, eventBusAdminExt, topicConfig, sleepTime, commandLine);
                } catch (Exception e) {
...
//            ServerUtil.printCommandLineHelp("mqadmin " + this.commandName(), options);
            ...
            eventBusAdminExt.shutdown();
        }
        return;
    }

    private void updateTopicPerm(String topic, EventBusAdminExt eventBusAdminExt, TopicConfig topicConfig, long sleepTime, CommandLine commandLine) throws InterruptedException, MQClientException, RemotingException, MQBrokerException {
        TopicRouteData topicRouteData = eventBusAdminExt.examineTopicRouteInfo(topic);
        ...
        if (commandLine.hasOption('b')) {
            ...
            brokerAddrs = fetchMasterAddrByTopic(eventBusAdminExt, topic);
        }
        ...
        for (String brokerAddr : brokerAddrs) {
            String brokerName = getBrokerNameByBrokerAddr(eventBusAdminExt, brokerAddr);
            ...
            eventBusAdminExt.createAndUpdateTopicConfig(brokerAddr, topicConfig);
            Thread.sleep(sleepTime);
            System.out.printf("update topic[%s] perm from %s to %s in %s success.%n", topic, oldPerm, perm, brokerAddr);
        }
    }

    private Set<String> fetchMasterAddrByTopic(EventBusAdminExt defaultMQAdminExt, String topic) throws RemotingException, MQClientException, InterruptedException {
    ...
    private String getBrokerNameByBrokerAddr(EventBusAdminExt eventBusAdminExt, String brokerAddr) throws InterruptedException, MQBrokerException, RemotingTimeoutException, RemotingSendRequestException, RemotingConnectException {
        String brokerName = null;
        if (clusterInfo == null) {
            clusterInfo = eventBusAdminExt.examineBrokerClusterInfo();
        }
