...
import com.gcote.eventbus.client.impl.producer.RRResponseFuture;
import com.gcote.eventbus.client.impl.producer.ResponseTable;
...
public class EventBusClientRemotingProcessor implements NettyRequestProcessor {
    public static final Logger LOGGER = LoggerFactory.getLogger(EventBusClientRemotingProcessor.class);
    private final EventBusClientInstance mqClientFactory;

    public EventBusClientRemotingProcessor(final EventBusClientInstance mqClientFactory) {
    ...
    public RemotingCommand processRequest(ChannelHandlerContext ctx, RemotingCommand request) throws Exception {
        switch (request.getCode()) {
            case EventBusRequestCode.PUSH_RR_REPLY_MSG_TO_CLIENT:
                return this.receiveRRReplyMsg(ctx, request);
            case EventBusRequestCode.NOTIFY_WHEN_TOPIC_CONFIG_CHANGE:
    ...
    private RemotingCommand receiveRRReplyMsg(ChannelHandlerContext ctx,
        ...
        try {
            ...
            MessageAccessor.putProperty(msg, EventBusMessageConst.ARRIVE_TIME, String.valueOf(receiveTime));
    ...
    private void processResponse(MessageExt msg) {
        ...
        final String uniqueId = msg.getUserProperty(EventBusConstant.PROPERTY_RR_REQUEST_ID);
